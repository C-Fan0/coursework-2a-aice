...
files, it will compile the .oat and .ll files to .s files (using the AICE2007
frontend and backend) and then combine the results with the .c and .o
files to produce an executable named a.out.  You can also compile the .ll files
...
* To compile oat files using the AICE2007 backend:
...
* Useful flags:
  ...
  | --execute-x86     | runs the resulting a.out file natively (applies to 4521/5521 backend or clang-compiled code)      |
  | --clang           | compiles to assembly using clang, not the 4521 backend                                            |
...
Run the test case cw2aprograms/fact.oat using the AICE2007 backend:

          /oatc --execute-x86 cw2aprograms/fact.oat bin/runtime.c 
          120--------------------------------------------------------------- Executing: a.out
          * a.out returned 0
