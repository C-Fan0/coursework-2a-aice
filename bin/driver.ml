...
let process_ll_ast path file ll_ast =
  ...
  (* Optionally interpret it using the CIS 4521/5521 reference interperter. *)
  ...
  let _ =
    if !clang then begin
      ...
      Platform.verb "* compiling with the Oat backend";
...
let parse_oat_file filename =
  let lexbuf = read_file filename |> 
               Lexing.from_string
  in
  Lexer.reset_lexbuf filename 1 lexbuf;  (* set the filename *)  
  try
