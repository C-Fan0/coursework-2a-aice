(* astlib.ml *)
(*  ------------------------------------------------------------------------- *)
(* Helper functions for abstract syntax of trees. --------------------------- *)
...
(* The following functions define a pretty printer for Oat v.1 syntax.  It
   Uses the OCaml Format library to ensure nice layout and indentation.  *)
...
and print_exp_aux level fmt e =
  ...
  begin match e.elt with
    ...
    | Lhs l -> print_lhs_aux level fmt l        
    | CArr (ty,vs) -> begin
    ...
    | Call (f, es) -> print_id_aux fmt f; print_exps_aux "(" ")" fmt es
    | NewArr (ty, e1) ->
...
and print_lhs_aux level fmt e =
  let pps = pp_print_string fmt in
  let ppsp = pp_print_space fmt in
  let this_level = 130 in
  if this_level < level then pps "(";
  begin match e.elt with
...
and print_stmt_aux fmt s =
  ...
  begin match s.elt with
    ...
    | Assn (p,e) ->
        pp_open_box fmt 0;
	    print_lhs_aux 0 fmt p;
	    pps " ="; ppsp ();
	    print_exp_aux 0 fmt e;
	    pps ";"; pp_close_box fmt ()

    | SCall (f, es) -> 
       print_id_aux fmt f; print_exps_aux "(" ")" fmt es; pps ";"
...
let string_of_lhs (l:lhs node) : string = string_of (print_lhs_aux 0) l
...
(* The following functions define another printer for abstract syntax as valid
   OCaml constructors.  These ml_string_of_* functions were used to generate
   the examples in [tests/progast.ml]. 
*)
...
let rec ml_string_of_exp_aux (e: exp) : string =
  begin match e with 
    ...
    | Lhs l -> sp "Lhs %s" (ml_string_of_lhs l)
    | Call (f, exps) -> sp "Call (%s, %s)"
                            (ml_string_of_id f)
...
and ml_string_of_lhs_aux (l:lhs) : string =
  begin match l with
...
and ml_string_of_lhs (l:lhs node) : string =
  ml_string_of_node ml_string_of_lhs_aux l
...
let rec ml_string_of_stmt_aux (s:stmt) : string =
  match s with
  | Assn (l, e) -> sp "Assn (%s,%s)" (ml_string_of_lhs l) (ml_string_of_exp e)
  | Decl d -> sp "Decl (%s)" (ml_string_of_vdecl_aux d)
  | Ret e -> sp "Ret (%s)" (ml_string_of_option ml_string_of_exp e)
  | SCall (f, exps) -> 
     sp "SCall (%s, %s)" (ml_string_of_id f) (ml_string_of_list ml_string_of_exp exps)
...
(* The following functions define an equivalence relation on abstract syntax.
   The equivalence relation *ignores* any location data and compares the
   asts only for the "real" constructors. *)
...
(* Ignores the .loc field and compare only the elements. *)
...
let rec eq_exp_aux (e1:exp) (e2:exp) : bool =
  begin match e1, e2 with
    ...
    | Lhs l1, Lhs l2 -> eq_lhs l1 l2
    | CArr (t1,cs1), CArr (t2,cs2) -> eq_ty t1 t2 && eq_list eq_exp cs1 cs2
    | Call (i1, es1), Call (i2, es2) -> i1 = i2 && eq_list eq_exp es1 es2
...
and eq_lhs_aux (l1:lhs) (l2:lhs) : bool =
  begin match l1, l2 with
  | Id x, Id y -> x = y
...
and eq_lhs (l1:lhs node) (l2:lhs node) : bool = eq_node eq_lhs_aux l1 l2
...
let rec eq_stmt_aux (s1: stmt) (s2: stmt) : bool =
  ...
  | Assn (l1, e1), Assn (l2, e2) -> eq_lhs l1 l2 && eq_exp e1 e2
  | Decl d1, Decl d2 -> eq_vdecl_aux d1 d2
  | Ret eo1, Ret eo2 -> eq_option eq_exp eo1 eo2
  | SCall (x1, es1), SCall (x2, es2) -> x1 = x2 && eq_list eq_exp es1 es2
